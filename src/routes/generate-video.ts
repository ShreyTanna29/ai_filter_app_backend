...
const textToVideoHandler = async (
  ...
  try {
    ...
    const isKling25TurboPro =
      /kling-v?2\.5-turbo-pro(?!.*image)|klingai:6@1/i.test(rawModel);
    ...
    if (!isVeo3 && !isPixverseV5 && !isKling25TurboPro) {
      ...
      res.status(400).json({
        success: false,
        error:
          'Only "veo 3" (google:3@0) and "pixverse v5" models are supported for text-to-video',
      });
      return;
    }
    ...
    if (isVeo3) {
      try {
        ...
        let veoStream;
        try {
          veoStream = await axios.get(videoUrl, {
            responseType: "stream",
            timeout: 180000,
          });
        } catch (e) {
        ...
        console.error("[Veo3 T2V] Fatal error:", err?.response?.data || err);
        await logAppApiCall(
          appId,
          "text-to-video",
          "video",
          ...
          for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            try {
              ...
              const pollItem = Array.isArray(pollData?.data)
                ? pollData.data.find((d: any) => d?.taskUUID === pollTaskUUID)
                : pollData?.data;

              console.log(
                `[Kling 2.5 Turbo Pro Text-to-Video] Poll attempt ${attempt}:`,
                {
              ...
              videoUrl =
                ...
                pollItem?.url ||
                pollItem?.video ||
                (Array.isArray(pollItem?.videos) ? pollItem.videos[0] : null);
              ...
              const pollStatus = pollItem?.status || pollItem?.taskStatus;
              if (
                pollStatus &&
              ...
              if (pollErr?.response?.status === 400) {
                consecutive400++;
                console.warn(
                  `[Kling 2.5 Turbo Pro Text-to-Video] Poll 400 error (${consecutive400}/3):`,
                  {
    ...
    if (isPixverseV5) {
      try {
        ...
        try {
          ...
          pixStream = await axios.get(videoUrl, {
            responseType: "stream",
            timeout: 180000,
          });
          console.log(
...
router.post("/text-to-video", requireApiKey, textToVideoHandler);
