...
router.post(
  "/photo-templates",
  requirePermission("photo_templates", "CREATE"),
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let mainCategory = await prisma.photo_Category.findFirst({
        where: { name },
      });
      if (!mainCategory) {
...
router.put(
  "/photo-templates/:id",
  requirePermission("photo_templates", "UPDATE"),
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { name, description, subcategories } = req.body;
...
router.delete(
  "/photo-templates/:id",
  requirePermission("photo_templates", "DELETE"),
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
